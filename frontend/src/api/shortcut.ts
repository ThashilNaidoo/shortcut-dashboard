...
export async function fetchStories(params?: { pageSize?: number; next?: string | null }) {
...
export async function fetchStoryById(storyId: number): Promise<StoryDTO> {
    const res = await fetch(`${ENDPOINT_BASE}/${storyId}`);
    if (!res.ok) throw new Error(`HTTP ${res.status}: ${await res.text()}`);

    return (await res.json()) as StoryDTO;
}